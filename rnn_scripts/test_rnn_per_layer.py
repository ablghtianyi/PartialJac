...
# chijs = chijs / n_ave
path = f'data/{norm_type}_{act_name}_d{n_hidden}_perlayer.npy'
...
if os.path.exists(path):
    ...
    for ave in tqdm(range(n_ave)):
        ...
        for i in range(wvars.shape[0]):
            for j in range(mus.shape[0]):
                model = RNNLike(n_channel * img_size**2, width, out_size, n_hidden, wvars[i]**0.5, 0.0, act_name=act_name, bias=bias, mu=mus[j], norm_type=norm_type).to(device=device)
                ghooks = ghook_rnn(model, bs=batch_size, n_proj=n_proj, device=device)
                out = model(xr)
                # print(len(ghooks))
                results = np.array([ghooks[i].pj.cpu().numpy() for i in ts])
                chijs[:, i, j] += results

                
    chijs = chijs / n_ave
    print(chijs[-1])
    np.save(path , {'chijs': chijs})
